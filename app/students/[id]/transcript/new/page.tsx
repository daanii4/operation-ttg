...
import TranscriptEntryForm from "./transcript-entry-form";
...
export default async function NewTranscriptPage({
  ...
  if (!session || session.userId === "anonymous") {
    redirect(`/login?redirectTo=${encodeURIComponent(`/students/${params.id}/transcript/new`)}`);
  }
  ...
  return (
    <DashboardShell omitAppHeader pageTitle="Add transcript course">
      <Breadcrumb
        items={[
          { label: "Cohort Dashboard", href: "/dashboard/analytics" },
          { label: fullName, href: `/students/${params.id}` },
          { label: "Add transcript course" },
        ]}
      />
      <div className="mt-6 max-w-2xl">
        <TranscriptEntryForm studentId={params.id} />
      </div>
    </DashboardShell>
  );
}
